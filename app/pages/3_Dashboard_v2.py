...
def _build_inlined_html_uncached(requested_date: str | None = None) -> str:
    ...
    overrides_js = (
        ...
        f"  window.IDX_DATA.TOP_BUYERS_BY_TICKER    = {json.dumps(payload.get('top_buyers_by_ticker', {}))};"
        f"  window.IDX_DATA.TOP_SELLERS_BY_TICKER   = {json.dumps(payload.get('top_sellers_by_ticker', {}))};"
        f"  window.IDX_DATA.BROKER_NET_BY_TICKER    = {json.dumps(payload.get('broker_net_by_ticker', {}))};"
        f"  window.IDX_DATA.PRICE_SERIES_BY_TICKER  = {json.dumps(payload.get('price_series_by_ticker', {}))};"
        f"  window.IDX_DATA.FLOW_SIGNALS_BY_TICKER  = {json.dumps(payload.get('flow_signals_by_ticker', {}))};"
        f"  window.IDX_DATA.SCORE_HISTORY_BY_TICKER = {json.dumps(payload.get('score_history_by_ticker', {}))};"
        f"}}"
        f"</script>"
    )
    html = html.replace(
